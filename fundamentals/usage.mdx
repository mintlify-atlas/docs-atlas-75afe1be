...
<ParamField path="estimatedItemSize" type="number">
  This is a single numeric value that hints FlashList about the approximate size of the items before they're rendered.
</ParamField>

<Note>
While not strictly required, providing `estimatedItemSize` can improve initial render performance. In v2, FlashList will measure items automatically, but providing an estimate helps with the initial layout.
</Note>
